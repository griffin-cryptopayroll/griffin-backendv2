...
func GenerateEmployee(c *gin.Context, db dao.GriffinWeb2Conn) {
	...
	argsQuery, msg, err := common.HandleOptionalQueryParamV2(c, args)
	if err != nil {
	...
	errPacketPF, err := common.ValidPayFreqV2(argsQuery[EMP_PAY_FREQ])
	if err != nil {
		c.JSON(http.StatusBadRequest, errPacketPF)
		return
	}
	errPacketET, err := common.ValidEmployTypeV2(argsQuery[EMPLOYEE_TYPE])
	if err != nil {
		c.JSON(http.StatusBadRequest, errPacketET)
		return
	}
	errPacketDT, err := common.ValidMonthDateV2(payday, argsQuery[EMP_PAY_FREQ])
	if err != nil {
		c.JSON(http.StatusBadGateway, errPacketDT)
		return
	}
	...
	if err != nil {
		msg := api_base.CommonResponse{
			Status:  false,
			Message: fmt.Sprintf("%s: %v", DATABASE_CREATE_FAIL, err),
		}
		c.JSON(http.StatusBadRequest, msg)
		return
	}
...
// @Param gid query string true "Employee's griffin id (in uuid form)"
// @Param employer_gid query string true "Employee's information. Corp Gid or Organization Gid"
// @Router /api/v1/employee/single [get]
...
// @Produce  json
// @Param employer_gid query string true "Employee's information. Corp Gid or Organization Gid"
// @Router /api/v1/employee/multi [get]
// @Success 200 {object} []ent.EMPLOYEE
// @Failure 400 {object} api_base.CommonResponse
// @Failure 500 {object} api_base.CommonResponse
